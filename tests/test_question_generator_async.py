...
async def test_generate_initial_question_with_mock(
    mock_call_openai_api_async, mock_openai_agent
):
    """
    Test generate_initial_question with mocked OpenAI API.
    """
    # Mock the API response
    mock_response = TextResponse(
        content="What are the implications of AI in education?"
    )
    mock_call_openai_api_async.return_value = mock_response

    # Call the method
    question = await mock_openai_agent.generate_initial_question(
        topic_name="Artificial Intelligence",
        context_text="Its impact on modern education systems.",
        complexity="advanced",
    )

    # Validate the response
    assert isinstance(question, str)
    assert question == "What are the implications of AI in education?"
    mock_call_openai_api_async.assert_awaited_once()
...
def mock_openai_agent():
    """
    Create a QuestionGeneratorAsync instance with a mocked OpenAI client.
    """
    mock_client = AsyncMock()
