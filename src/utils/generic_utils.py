...
def insert_file_name_suffix(file_path: Union[str, Path], suffix: str) -> Path:
    """
    Inserts a suffix at the end of a file name, before the file extension,
    and returns the full path with the modified file name.

    Args:
        file_path (Union[str, Path]): The original file path as a string or Path object.
        suffix (str): The suffix to add to the file name. Should start with an underscore
                      or other delimiter if separation from the original name is desired.

    Returns:
        Path: The full path with the modified file name, including the suffix.

    Example:
        >>> insert_suffix("data/output.json", "_modified")
        PosixPath('data/output_modified.json')

        >>> insert_suffix(Path("logs/error.log"), "_v2")
        PosixPath('logs/error_v2.log')

    Notes:
        - If `file_path` is a string, it is converted to a `Path` object.
        - This function only modifies the file name, keeping the original directory path intact.
    """
    # Ensure file_path is a Path object if it's provided as a string
    file_path = Path(file_path)

    # Create the new filename with the suffix before the file extension
    new_file_name = f"{file_path.stem}{suffix}{file_path.suffix}"

    # Combine the parent directory with the new filename for the full path
    full_path = file_path.parent / new_file_name

    logger.info(f"Updated file path: {full_path}")
    return full_path
